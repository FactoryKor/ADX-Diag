...
class Config:
    ...
    demo: bool = False
    format: str = "html"          # html(사람용) | json(SRE Agent/MCP용, stdout)
...
class Analyzer:
    ...
    def _slow_queries(self):
        if not self.e.slow:
            self.findings.append(Finding(SEV_INFO, "쿼리", "느린 쿼리 미평가",
                "엔진 계층(.show queries) 데이터가 없습니다(--database 미지정 또는 권한).",
                "--database 지정 + 권한 확인 후 다시 실행하면 쿼리별 상세가 수집됩니다."))
            return
        top = self.e.slow[0]
        if top.duration_s and top.duration_s >= 10:
            ...
            self.findings.append(Finding(SEV_OK, "쿼리", "느린 쿼리 없음",
                f"상위 쿼리 지속시간이 임계(10s) 미만입니다(최대 {top.duration_s or 0:.1f}s).",
                "조치 불필요."))
    ...
    def _cache(self):
        ...
        if v is None:
            self.findings.append(Finding(SEV_INFO, "캐시", "캐시 사용률 미평가",
                "CacheUtilization 메트릭이 없어 평가하지 못했습니다.",
                "--resource-id/--region 로 Azure Monitor 메트릭을 수집해 다시 실행하세요."))
        elif v >= 90:
            ...
            self.findings.append(Finding(SEV_OK, "캐시", f"캐시 사용률 양호 ({v}%)",
                "핫캐시 사용률이 임계(90%) 미만입니다.", "조치 불필요."))

    def _capacity(self):
        tq = self._metric("TotalNumberOfThrottledQueries")
        if tq is None:
            self.findings.append(Finding(SEV_INFO, "용량", "쿼리 스로틀 미평가",
                "TotalNumberOfThrottledQueries 메트릭이 없어 평가하지 못했습니다.",
                "Azure Monitor 메트릭(--resource-id/--region)을 수집해 다시 실행하세요."))
        elif (tq.max_v or 0) > 0:
            ...
            self.findings.append(Finding(SEV_OK, "용량", "쿼리 스로틀 없음",
                "스로틀된 쿼리가 없습니다.", "조치 불필요."))
    ...
    def _ingestion(self):
        ...
        if v is None:
            self.findings.append(Finding(SEV_INFO, "인제스트", "인제스트 지연 미평가",
                "IngestionLatencyInSeconds 메트릭이 없어 평가하지 못했습니다.",
                "Azure Monitor 메트릭을 수집해 다시 실행하세요."))
        elif v >= 60:
            ...
            self.findings.append(Finding(SEV_OK, "인제스트", f"인제스트 지연 양호 ({v:.0f}s)",
                "인제스트 지연이 임계(60s) 미만입니다.", "조치 불필요."))
        if self.e.ingestion_failures > 0:
            ...
            self.findings.append(Finding(SEV_OK, "인제스트", "인제스트 실패 없음",
                "최근 24h 인제스트 실패가 없습니다.", "조치 불필요."))

    def _extents(self):
        ex = self.e.extents
        if not ex or ex.get("count") is None:
            self.findings.append(Finding(SEV_INFO, "스토리지", "익스텐트 미평가",
                "익스텐트 정보를 가져오지 못했습니다(--database 미지정 또는 권한).",
                "--database 지정 + 권한 확인 후 다시 실행하세요."))
        elif ex["count"] >= 10000:
            ...
            self.findings.append(Finding(SEV_OK, "스토리지", f"익스텐트 수 양호 ({int(ex['count']):,}개)",
                "익스텐트 수가 임계(10000) 미만입니다.", "조치 불필요."))

    def _cpu(self):
        cpu = self._metric("CPU")
        v = cpu.max_v if cpu else None
        if v is None:
            self.findings.append(Finding(SEV_INFO, "리소스", "CPU 미평가",
                "CPU 메트릭이 없어 평가하지 못했습니다.",
                "Azure Monitor 메트릭을 수집해 다시 실행하세요."))
        elif v >= 90:
            ...
            self.findings.append(Finding(SEV_OK, "리소스", f"CPU 양호 ({v}%)",
                "CPU 최대치가 임계(90%) 미만입니다.", "조치 불필요."))
    ...
    def _query_duration(self):
        ...
        if v is None:
            self.findings.append(Finding(SEV_INFO, "쿼리 · 메트릭", "쿼리 지속시간 미평가",
                "QueryDuration 메트릭이 없어 평가하지 못했습니다.",
                "Azure Monitor 메트릭을 수집해 다시 실행하세요."))
            return
        sec = v / 1000.0
        if sec < 10:
            self.findings.append(Finding(SEV_OK, "쿼리 · 메트릭",
                f"쿼리 지속시간 양호 (최대 {sec:.1f}s)",
                "QueryDuration 최대치가 임계(10s) 미만입니다.", "조치 불필요."))
            return
    ...
    def _ingestion_utilization(self):
        ...
        if v is None:
            self.findings.append(Finding(SEV_INFO, "인제스트 · 메트릭", "인제스트 사용률 미평가",
                "IngestionUtilization 메트릭이 없어 평가하지 못했습니다.",
                "Azure Monitor 메트릭을 수집해 다시 실행하세요."))
        elif v >= 80:
            ...
            self.findings.append(Finding(SEV_OK, "인제스트 · 메트릭", f"인제스트 사용률 양호 ({v}%)",
                "인제스트 사용률이 임계(80%) 미만입니다.", "조치 불필요."))
...
_SEV_ORDER = {SEV_CRIT: 0, SEV_WARN: 1, SEV_INFO: 2, SEV_OK: 3}


def severity_counts(findings):
    counts = {SEV_CRIT: 0, SEV_WARN: 0, SEV_INFO: 0, SEV_OK: 0}
    for f in findings:
...
def recommended_actions(findings):
    """우선순위(심각도)로 정렬한 중복 제거 조치 목록 — SRE Agent가 'next steps'로 표시."""
    seen = set()
    actions = []
    for f in sorted(findings, key=lambda x: _SEV_ORDER.get(x.severity, 9)):
        if not f.recommendation or f.recommendation in seen:
            continue
        seen.add(f.recommendation)
        actions.append({"severity": f.severity, "category": f.category,
                        "title": f.title, "action": f.recommendation})
    return actions


def summary_text(findings, score, target):
    """SRE Agent / 채팅 표면용 자연어 한 줄 요약."""
    c = severity_counts(findings)
    worst = worst_severity(findings)
    s = (f"Health {score}/100 ({SEV_LABEL.get(worst, worst)}). "
         f"위험 {c[SEV_CRIT]}, 주의 {c[SEV_WARN]}, 정보 {c[SEV_INFO]}건"
         + (f" — {target}" if target else "") + ".")
    top = next((f for f in sorted(findings, key=lambda x: _SEV_ORDER.get(x.severity, 9))
                if f.severity in (SEV_CRIT, SEV_WARN)), None)
    if top:
        s += f" 핵심 이슈: {top.title}."
...
def render_html(cfg, eng: EngineData, metrics, findings, baseline, generated_at):
    ...
    if eng.capacity:
        ...
        cap_html = (f'<table><thead><tr><th>리소스</th><th>사용량</th><th>전체</th>'
    ...
    if baseline:
        ...
        base_html = (f'<p class="kv">이전 스냅샷 <b>{_esc(baseline.get("generated_at"))}</b> 대비 · '
                     f'건강 점수 <b>{prev} → {score}</b> '
...
<header><div class="hd">
  <div><h1>ADX 진단 리포트 — Azure Data Explorer<span class="lvl">전체 + 회귀</span></h1>
    <div class="meta">클러스터 <b>{_esc(cfg.cluster or 'demo')}</b>
...
<div class="wrap">
  <div class="summary">
    <div class="stat crit"><div class="n">{nc}</div><div class="l">위험</div></div>
    <div class="stat warn"><div class="n">{nw}</div><div class="l">주의</div></div>
    <div class="stat info"><div class="n">{ni}</div><div class="l">정보</div></div>
  </div>
...
# ──────────────────────────────────────────────────────────────────────────
# JSON 출력 (SRE Agent / MCP 연동용) — pg_diagnose 와 동일 스키마/새니타이즈
# ──────────────────────────────────────────────────────────────────────────
...
def emit_json(findings, score, target, tool="adx_diagnose"):
    payload = {
        "tool": tool,
        "target": target,
        "health_score": score,
        ...
        "summary": _clean(summary_text(findings, score, target)),
        "findings": [{k: _clean(val) for k, val in asdict(f).items()} for f in findings],
        "recommended_actions": _clean(recommended_actions(findings)),
    }
...
def parse_args(argv):
    ...
    p.add_argument("--format", choices=["html", "json"], default="html",
                   help="html(기본, 사람용) | json(SRE Agent/MCP용, stdout)")
    a = p.parse_args(argv)
    return Config(cluster=a.cluster, database=a.database, auth=a.auth, app_id=a.app_id, tenant=a.tenant,
                  ...
                  history_dir=a.history_dir, out=a.out, demo=a.demo, format=a.format)
...
def main(argv=None):
    ...
    if getattr(cfg, "format", "html") == "json":
        emit_json(findings, score, cfg.cluster or ("demo" if cfg.demo else ""))
        return 0
